...
def get_local_process(sas_code):
	f = open('/home/jingyuz/tmp.sas', 'w+')
	f.write(sas_code)
	f.close()

	p=subprocess.Popen(['sas', '-sysin', '/home/jingyuz/tmp.sas', '-nolog'], stdout=subprocess.PIPE,stderr=subprocess.PIPE)
	ret = p.communicate()[0].decode('latin1')
	os.remove('/home/jingyuz/tmp.sas')
	return ret
...
def get_table_sql(table_name, fpath, schema, drop="", rename="", return_sql=True):
    sas_template = """
		...
		* basic table infor is year2000 in table work.schema;
...
def get_wrds_process(table_name, fpath, schema, wrds_id, drop="",
...
def wrds_process_to_pg(table_name, schema, engine, p):
    ...
    copy_cmd =  "COPY public." + table_name + " (" + ", ".join(var_names) + ")"
