...
- SOX 302 Disclosure Controls (`auditsox302`): To be renamed `feed10_sox_302_disclosure_contro`?
- SOX 404 Internal Controls (`auditsox302`): To be renamed `feed11_sox_404_internal_controls`?
...
In each case, the MCCGR database includes a table with the same name, but in general, we do not include *all* variables provided in the WRDS version of the table.
...
- `feed17_director_and_officer_chan`: Text field is currently omitted (see below).
...
The `feed17_director_and_officer_chan` text variables currently do not work due to the text column being too wide.
