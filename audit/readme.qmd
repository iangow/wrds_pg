...
Here `CROSS JOIN LATERAL` evaluates `UNNEST(...)` once for each row of `feed40_comment_letter_threads`.
That lets the array-building expressions refer to columns from the current row `t`, and it returns one output row per unnested element rather than one wide row containing arrays.
...
The same general pattern applies to several other Audit Analytics tables:
